...
4. Optionally install =yasnippet= and/or =projectile= (=project.el= is also supported), if you want the additional yankpad features that those package provide.
5. After editing your =yankpad-file=, make sure to run =M-x yankpad-reload=. Optionally you can add =yankpad-reload= to the =after-save-hook= of the =yankpad-file=. See the end of the example file above!
6. That's it!
...
6. Yankpad caches your snippets, making it a bit snappier to insert snippets from the yankpad. If you've edited your =yankpad-file= you need to use =M-x yankpad-reload= to rebuild the snippet caches. See the end of the example =yankpad-file= (in the beginning of this README) on how to do this automatically when saving.
...
- 2.50 (May 2026) :: Changed the caching mechanisms to make yankpad snappier. Thanks [[https://github.com/isamert][isamert]]! Make sure to run =yankpad-reload= after editing your =yankpad-file=. See the end of the example =yankpad-file= in the beginning of this README on how to reload the file automatically upon saving.
